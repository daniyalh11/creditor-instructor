...
const sampleDebates = [
  {
    ...
    submissions: [
      ...
      },
      {
        ...
        position: 'against' as const,
        response: 'Technology addiction is real and affecting mental health, especially among young people.',
        score: 38,
...
const DebateInstructorPage = () => {
  const { moduleId, debateId } = useParams<{ moduleId: string; debateId: string }>();
  const navigate = useNavigate();
  const [debate, setDebate] = useState(sampleDebates.find(d => d.id === debateId) || sampleDebates[0]);

  const handleBack = () => {
    navigate(`/courses/modules/${moduleId}/assessments`);
  };

  const handleDebateUpdate = (updatedDebate: any) => {
