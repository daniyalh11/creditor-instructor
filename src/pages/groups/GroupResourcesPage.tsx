...
const GroupResourcesPage = () => {
  ...
  const handleAddResource = (resourceData: { title: string; type: 'file' | 'link'; content: string; description?: string }) => {
  ...
  const getFileTypeIcon = (type: string, isLink: boolean = false) => {
    if (isLink) return <ExternalLink className="h-5 w-5 text-blue-500" />;
    return <FileText className="h-5 w-5 text-blue-500" />;
  };

  const getFileTypeBadge = (type: string, isLink: boolean = false) => {
