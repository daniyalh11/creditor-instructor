...
const CourseBuilder = () => {
  ...
  useEffect(() => {
    ...
    setModules(prevModules => 
      prevModules.map(module => {
        const moduleUnits = savedUnits.filter((unit: any) => unit.moduleId === module.id);
        const moduleAssessments = savedAssessments.filter((assessment: any) => assessment.moduleId === module.id);
  ...
  const handleDeleteModule = (moduleId: number) => {
    ...
    if (module && window.confirm(`Are you sure you want to delete "${module.title}"?`)) {
      ...
      const filteredUnits = savedUnits.filter((unit: any) => unit.moduleId !== moduleId);
      const filteredAssessments = savedAssessments.filter((assessment: any) => assessment.moduleId !== moduleId);
  ...
  const handleCreateUnits = (moduleId: number, moduleName: string) => {
    navigate(`/courses/builder/${courseId}/modules/${moduleId}/units`);
  };

  const handleCreateAssessments = (moduleId: number, moduleName: string) => {
  ...
  const handlePublishCourse = () => {
    ...
    const courseData = {
      ...
      modules: modules.map(module => ({
        ...module,
        units: savedUnits.filter((unit: any) => unit.moduleId === module.id),
        assessments: savedAssessments.filter((assessment: any) => assessment.moduleId === module.id)
      })),
    ...
    const filteredDrafts = draftCourses.filter((course: any) => course.id !== courseId);
