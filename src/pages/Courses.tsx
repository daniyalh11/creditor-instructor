...
const Courses = () => {
  ...
  const [courseType, setCourseType] = useState<'open' | 'sequential'>('open');
  ...
  useEffect(() => {
    ...
    const transformedPublishedCourses = publishedCourses.map((course: any) => ({
      id: course.id,
      title: course.title,
      description: course.description || 'Published course with comprehensive content',
      students: course.students || 0,
      duration: course.modules?.reduce((total: number, module: any) => {
  ...
  const handleCourseEdit = (courseId: number, courseName: string) => {
    toast({
      title: "Edit Course",
  ...
  const handleCourseArchive = (courseId: number, courseName: string) => {
  ...
  const handleCourseDelete = (courseId: number, courseName: string) => {
    setCourses(prevCourses => 
      prevCourses.map(course => 
  ...
  const handleCourseRestore = (courseId: number, courseName: string) => {
    setCourses(prevCourses => 
      prevCourses.map(course => 
  ...
  const handleAddToCatalog = (courseId: number, courseName: string, catalogName: string) => {
    setCourses(prevCourses => 
      prevCourses.map(course => 
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Tabs value={courseType} onValueChange={(value) => setCourseType(value as 'open' | 'sequential')}>
