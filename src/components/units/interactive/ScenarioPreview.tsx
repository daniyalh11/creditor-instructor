...
export const ScenarioPreview = ({ data, onClose }: ScenarioPreviewProps) => {
  ...
  const handleStart = () => {
    // Validate that there are scenes with content
  ...
  const handleResponseClick = (response: ScenarioResponse) => {
    setSelectedResponse(response.id);
    setShowFeedback(true);

    // Handle navigation after a short delay to show feedback
    setTimeout(() => {
      handleNavigation(response);
    }, 1500);
  };

  const handleNavigation = (response: ScenarioResponse) => {
    switch (response.nextAction) {
      case 'next-content':
        if (currentContentIndex < currentScene.contents.length - 1) {
          setCurrentContentIndex(currentContentIndex + 1);
        } else {
          // Move to next scene's first content
      ...
      case 'specific-content':
        // Find the specific content block
