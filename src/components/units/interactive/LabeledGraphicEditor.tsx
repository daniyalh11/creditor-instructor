...
type LabeledGraphicEditorProps = {
  isOpen: boolean;
  onClose: () => void;
...
export const LabeledGraphicEditor = ({ isOpen, onClose, onSave, initialData }: LabeledGraphicEditorProps) => {
  ...
  const [hotspots, setHotspots] = useState<Hotspot[]>(initialData?.hotspots || []);
  ...
  const handleEditHotspot = (hotspot: Hotspot, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const saveHotspot = () => {
    if (newHotspot.label && newHotspot.description && newHotspot.id) {
      setHotspots([...hotspots, newHotspot as Hotspot]);
      setNewHotspot({});
      setShowingHotspotForm(false);
    }
  };
