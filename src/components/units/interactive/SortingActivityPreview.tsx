...
type SortingActivityPreviewProps = {
  title?: string;
  instructions?: string;
  items?: SortingItem[];
  categories?: Category[];
...
export const SortingActivityPreview = ({ 
  ...
  onEdit
}: SortingActivityPreviewProps) => {
  const [availableCards, setAvailableCards] = useState<SortingItem[]>([...items]);
  const [categorizedItems, setCategorizedItems] = useState<{ [categoryId: string]: SortingItem[] }>({});
  const [draggedCard, setDraggedCard] = useState<SortingItem | null>(null);
  const [showResults, setShowResults] = useState(false);

  const handleDragStart = (e: React.DragEvent, card: SortingItem) => {
    setDraggedCard(card);
  ...
  const renderCard = (card: SortingItem, isCorrect?: boolean) => (
    <div
      key={card.id}
      draggable
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 space-y-4 bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen">
      ...
      {/* Available Cards Stack - Only show if there are cards available or if results are not shown */}
