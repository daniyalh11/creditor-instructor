...
export const TimelineEditor = ({ isOpen, onClose, onSave, initialData }: TimelineEditorProps) => {
  ...
  const triggerImageUpload = (eventIndex: number) => {
  ...
  const handleImageUpload = (eventIndex: number, event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleDateSelect = (eventIndex: number, date: Date | undefined) => {
    if (date) {
      updateEvent(eventIndex, 'date', format(date, 'yyyy-MM-dd'));
    }
  };

  const startRecording = async (eventIndex: number) => {
    try {
  ...
  const stopRecording = (eventIndex: number) => {
