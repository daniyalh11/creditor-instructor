...
type SortingActivityEditorProps = {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: SortingActivityData) => void;
  initialData?: Partial<SortingActivityData>;
};
...
const DEFAULT_ITEMS: SortingItem[] = [
...
export const SortingActivityEditor = ({ isOpen, onClose, onSave, initialData }: SortingActivityEditorProps) => {
  ...
  const [items, setItems] = useState<SortingItem[]>(DEFAULT_ITEMS);
  ...
  const updateCategory = (id: string, field: keyof Category, value: string) => {
    ...
    // Update items if category name changed
  ...
  const updateItem = (id: string, field: keyof SortingItem, value: string) => {
    setItems(items.map(item => 
  ...
  const handleSave = () => {
    const sortingData: SortingActivityData = {
      interactiveType: 'sorting-activity',
      title,
      instructions,
      categories,
