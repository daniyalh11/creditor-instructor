...
export const AccordionEditor = ({ isOpen, onClose, onSave, initialData }: AccordionEditorProps) => {
  ...
  const triggerImageUpload = (sectionIndex: number) => {
  ...
  const triggerVideoUpload = (sectionIndex: number) => {
  ...
  const handleVideoUpload = (sectionIndex: number, event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e) => {
