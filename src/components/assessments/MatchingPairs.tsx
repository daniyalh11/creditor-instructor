...
const MatchingPairs = () => {
  ...
  const handleTechClick = (techId: string) => {
    if (matches[techId]) return; // Already matched
    setSelectedTech(selectedTech === techId ? null : techId);
    setSelectedDesc(null);
  };

  const handleDescClick = (descId: string) => {
    if (Object.values(matches).includes(descId)) return; // Already used
  ...
  const isMatchUsed = (descId: string) => Object.values(matches).includes(descId);
  const isTechMatched = (techId: string) => matches[techId] !== undefined;
