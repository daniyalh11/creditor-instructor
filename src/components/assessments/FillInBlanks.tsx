...
const FillInBlanks = () => {
  ...
  const renderQuestionText = (question: Question) => {
    ...
    question.blanks.forEach(blank => {
      const inputKey = `${currentQuestion}-${blank.id}`;
      const inputHtml = `<input class="inline-input" data-blank-id="${blank.id}" placeholder="Blank ${blank.id.replace('BLANK', '')}" value="${answers[inputKey] || ''}" />`;
      text = text.replace(`___${blank.id}___`, inputHtml);
    });
    return text;
  };
