...
const ScenarioSimulation = () => {
  ...
  const scenario = {
    ...
    situation: "You are the Lead Database Administrator for an e-commerce company. It's Black Friday, and your primary database server has started showing critical performance issues.",
    metrics: [
  ...
  const calculateEffectiveness = () => {
    ...
    const selectedActionObjects = availableActions.filter(action => 
      selectedActions.includes(action.id)
    );
    
    // Calculate combined effectiveness (not just sum, as some actions complement each other)
    let totalEffectiveness = 0;
    selectedActionObjects.forEach(action => {
      totalEffectiveness += action.effectiveness;
    });
    
    // Cap at 100% and apply diminishing returns for multiple actions
    return Math.min(100, totalEffectiveness * 0.8);
  };
          ...
          <CardContent className="p-8">
            ...
            <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
              <p className="text-sm text-yellow-700">
                <span className="font-medium">Instructions:</span> Analyze the situation and select appropriate actions to resolve the database crisis. Consider the risks, costs, and effectiveness of each action. You can select multiple actions to execute simultaneously.
              </p>
            </div>
          </CardContent>
        </Card>
