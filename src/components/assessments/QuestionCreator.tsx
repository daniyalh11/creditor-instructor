...
export const QuestionCreator = ({ questionType, editingQuestion, onSave, onCancel }: QuestionCreatorProps) => {
  ...
  useEffect(() => {
    if (editingQuestion) {
      ...
      if (editingQuestion.correctAnswers) {
        setCorrectAnswers(editingQuestion.correctAnswers);
      } else {
        setCorrectAnswers([0]);
      }
  ...
  const isFormValid = () => {
    ...
    // For MCQ and Multiple Response questions, check if options are filled
    ...
    // For True/False questions, ensure a correct answer is selected
    if (questionType === 'true-false' && correctAnswers.length === 0) {
      return false;
    }

    // For matching questions, ensure pairs are filled
    ...
    // For fill-blank questions, ensure question text contains ___BLANK___
    ...
    <div className="h-full flex flex-col bg-white overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-6 space-y-6">
          ...
          {/* Image Upload Section - Only for non-matching questions */}
