...
export function ZoomClassesSection() {
  ...
  const [selectedRecording, setSelectedRecording] = useState<ZoomClass | null>(null);
  ...
  const handleScheduleClass = () => {
    ...
    const newZoomClass: ZoomClass = {
      ...
      zoomLink: newClass.zoomLink || `https://zoom.us/j/${Math.random().toString().substr(2, 9)}`,
      meetingId: newClass.meetingId || Math.random().toString().substr(2, 9).replace(/(.{3})/g, '$1 ').trim(),
      attendance: 0,
      totalStudents: 25,
      isCompleted: false
    };
  ...
  const handleJoinZoom = (zoomLink: string, title: string) => {
    window.open(zoomLink, '_blank');
    toast.success(`Joining ${title} on Zoom`);
  };

  const handleViewRecording = (cls: ZoomClass) => {
    setSelectedRecording(cls);
    setIsRecordingDialogOpen(true);
  };

  const handleDownloadRecording = (cls: ZoomClass) => {
    if (cls.recordingUrl) {
      // Simulate download
