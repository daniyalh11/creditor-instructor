...
const AssessmentCreator: React.FC<AssessmentCreatorProps> = ({ 
  ...
  const [questions, setQuestions] = useState<Question[]>(editingAssessment?.questions || []);
  ...
  const handleSaveAssessment = (questions: Question[], timeLimit: number) => {
    const assessment = {
  ...
  const handlePublishAssessment = (questions: Question[], timeLimit: number) => {
    const assessment = {
      id: editingAssessment?.id || Date.now(),
      ...assessmentDetails,
      questions,
