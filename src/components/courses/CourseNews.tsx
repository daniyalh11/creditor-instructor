...
const CourseNews = () => {
  ...
  const [activeView, setActiveView] = useState<'feed' | 'post' | 'announcement'>('feed');
  const [postContent, setPostContent] = useState('');
  const [isSticky, setIsSticky] = useState(false);
  const [posts, setPosts] = useState<{id: number, author: string, content: string, timestamp: Date, liked: boolean, avatar?: string}[]>([
    {
      id: 1,
  ...
  return (
    <div className="w-full animate-fade-in flex">
      ...
      {/* Right sidebar with activity and admin info */}
