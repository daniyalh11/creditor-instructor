...
const initialCourseData: CourseStructure = {
  id: 'new-123456',
...
const CourseBuilder = () => {
  ...
  const [course, setCourse] = useState<CourseStructure>(initialCourseData);
  const [autoSaveStatus, setAutoSaveStatus] = useState<'saved' | 'saving' | 'error'>('saved');
  const [draggedModule, setDraggedModule] = useState<string | null>(null);
  const [draggedUnit, setDraggedUnit] = useState<{moduleId: string, unitId: string} | null>(null);
  const [draggedLesson, setDraggedLesson] = useState<{moduleId: string, unitId: string, lessonId: string} | null>(null);
  ...
  // Auto-save whenever course data changes
  React.useEffect(() => {
  ...
  const handleUnitChange = (moduleId: string, unitId: string, field: keyof Unit, value: string) => {
    setCourse(prev => ({
      ...prev,
      modules: prev.modules.map(module => 
        module.id === moduleId 
  ...
  const handleAddLesson = (moduleId: string, unitId: string) => {
    setCourse(prev => ({
      ...prev,
      modules: prev.modules.map(module => 
        module.id === moduleId 
  ...
  const handleDeleteLesson = (moduleId: string, unitId: string, lessonId: string) => {
    setCourse(prev => ({
      ...prev,
      modules: prev.modules.map(module => 
        module.id === moduleId 
  ...
  const handleLessonChange = (moduleId: string, unitId: string, lessonId: string, field: keyof Lesson, value: any) => {
    setCourse(prev => ({
      ...prev,
      modules: prev.modules.map(module => 
        module.id === moduleId 
  ...
  const handleAddAttachment = (moduleId: string, unitId: string, lessonId: string, file: File) => {
  ...
  const handleDeleteAttachment = (moduleId: string, unitId: string, lessonId: string, attachmentId: string) => {
    setCourse(prev => ({
      ...prev,
      modules: prev.modules.map(module => 
        module.id === moduleId 
  ...
  const handleDragStart = (e: React.DragEvent, moduleId: string, unitId?: string, lessonId?: string) => {
    if (lessonId && unitId) {
  ...
  const handleUnitDrop = (e: React.DragEvent, targetModuleId: string, targetUnitId: string) => {
    e.preventDefault();
    if (draggedUnit) {
  ...
  const handleLessonDrop = (e: React.DragEvent, targetModuleId: string, targetUnitId: string, targetLessonId: string) => {
    e.preventDefault();
    if (draggedLesson) {
      if (draggedLesson.moduleId === targetModuleId && 
          draggedLesson.unitId === targetUnitId && 
          draggedLesson.lessonId !== targetLessonId) {
        // Reorder lessons within the same unit
        const module = course.modules.find(m => m.id === targetModuleId);
        ...
        // Move lesson between units in the same module
        const module = course.modules.find(m => m.id === targetModuleId);
  ...
  const handleCourseChange = (field: keyof CourseStructure, value: any) => {
    setCourse(prev => ({
      ...prev,
      [field]: value
    }));
                                                    ...
                                                    <div className="grid grid-cols-1 gap-4 pb-2">
                                                      ...
                                                      <div className="grid grid-cols-2 gap-4">
                                                        <div>
                                                          ...
                                                          <select
                                                            value={lesson.type}
                                                            onChange={(e) => handleLessonChange(module.id, unit.id, lesson.id, 'type', e.target.value as Lesson['type'])}
                                                      ...
                                                      {/* Lesson content based on type - Now supporting multiple content types in one lesson */}
                                                      <div className="space-y-4 border-t pt-4">
                                                        <div className="flex items-center justify-between">
                                                          <h5 className="text-sm font-medium">Lesson Content</h5>
                                                        </div>
                                                        
                                                        {/* Article/Text content section - Always show regardless of lesson type */}
                                                        ...
                                                        {/* Video content section - Always show regardless of lesson type */}
                                                        <div className="border rounded-md p-4">
                                                          <label className="text-sm font-medium mb-2 block flex items-center gap-2">
                                                        ...
                                                        {/* Assignment instructions - Only show for Assignment type lessons */}
                                                        ...
                                                        {/* Attachments section - Always show for all lesson types */}
                                                        <div className="border rounded-md p-4">
                                                          <label className="text-sm font-medium mb-2 block flex items-center gap-2">
