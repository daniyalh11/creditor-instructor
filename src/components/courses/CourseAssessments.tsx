...
// Sample survey data with proper structure
...
const CourseAssessments = () => {
  ...
  useEffect(() => {
    const loadAssessments = () => {
      ...
      // Check if this is a published course
      const publishedCourses = JSON.parse(localStorage.getItem('courses') || '[]');
      const publishedCourse = publishedCourses.find((course: any) => course.id === courseId);
      
      if (publishedCourse) {
        setIsPublishedCourse(true);
        
        // Collect all assessments from all modules
        const allAssessments: Assessment[] = [];
        
        if (publishedCourse.modules) {
          publishedCourse.modules.forEach((module: any) => {
        ...
        // Load default assessments for predefined courses
  ...
  return (
    <div className="p-6 animate-fade-in">
      ...
      <div className="space-y-8">
        ...
        {/* Empty state for when no assessments exist */}
