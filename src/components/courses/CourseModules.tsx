...
// Course module type - updated to match ModuleCard interface
type Module = {
  id: number;
  title: string;
  description: string;
...
const CourseModules = () => {
    ...
    const loadModulesData = () => {
      ...
      if (publishedCourse) {
        ...
        if (publishedCourse.modules && publishedCourse.modules.length > 0) {
          ...
          const courseModules = publishedCourse.modules.map((module: any, index: number) => ({
            id: module.id,
            title: module.title,
  ...
  const handleModuleAdd = (newModule: Module) => {
