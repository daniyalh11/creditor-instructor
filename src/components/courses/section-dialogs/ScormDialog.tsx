...
interface ScormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const ScormDialog = ({ open, onOpenChange }: ScormDialogProps) => {
  const { toast } = useToast();
  const [selectedOption, setSelectedOption] = useState<'new' | 'library' | null>(null);

  const handleOptionSelect = (option: 'new' | 'library') => {
