...
interface CourseGridProps {
  courses: Course[];
  onCourseClick?: (courseId: string, title: string) => void;
  onCourseEdit?: (courseId: string, title: string) => void;
  onCourseDelete?: (courseId: string, title: string) => void;
  onCourseArchive?: (courseId: string, title: string) => void;
  courseType?: 'sequential' | 'open';
}

const CourseGrid = ({ courses, onCourseClick, onCourseEdit, onCourseDelete, onCourseArchive, courseType = 'open' }: CourseGridProps) => {
  const navigate = useNavigate();

  const handleCourseClick = (courseId: string, title: string, event: React.MouseEvent) => {
  ...
  const handleCourseEdit = (courseId: string, title: string) => {
    if (onCourseEdit) {
      onCourseEdit(courseId, title);
    }
  };

  const handleCourseDelete = (courseId: string, title: string) => {
    if (onCourseDelete && window.confirm(`Are you sure you want to delete "${title}"?`)) {
      onCourseDelete(courseId, title);
    }
  };

  const handleCourseArchive = (courseId: string, title: string) => {
